            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody id="infraccionesTableBody">
                        ...
                        <tr>
                            <td><strong>{{ $infraccion->numero_acta ?? 'N/A' }}</strong></td>
                            <td>{{ $infraccion->fecha ? $infraccion->fecha->format('d/m/Y') : 'N/A' }}</td>
                            <td>{{ $infraccion->conductor_nombre ?? 'No especificado' }}</td>
                            <td>{{ $infraccion->placa ?? 'N/A' }}</td>
                            <td>{{ $infraccion->tipo_infraccion ?? 'No especificada' }}</td>
                            <td>
                                <span class="badge bg-{{ 
                                    ($infraccion->estado ?? 'pendiente') === 'resuelta' ? 'success' : 
                                    (($infraccion->estado ?? 'pendiente') === 'en_proceso' ? 'warning' : 'danger') 
                                }}">
                                    {{ ucfirst($infraccion->estado ?? 'Pendiente') }}
                                </span>
                            </td>
                            <td>{{ $infraccion->inspector_nombre ?? 'No asignado' }}</td>
                            <td>
